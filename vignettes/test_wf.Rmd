...
# Set cal levels
cal1 <- selectROI(cal_mix, mz = 287.4, contrast.enhance="histogram")
image(cal_mix, cal1~x*y, key=T)

cal2 <- selectROI(cal_mix, mz = 287.4, contrast.enhance="histogram")
image(cal_mix, cal2~x*y, key=T)

cal3 <- selectROI(cal_mix, mz = 287.4, contrast.enhance="histogram")
image(cal_mix, cal3~x*y, key=T)

cal4 <- selectROI(cal_mix, mz = 287.4, contrast.enhance="histogram")
image(cal_mix, cal4~x*y, key=T)

cal5 <- selectROI(cal_mix, mz = 287.4, contrast.enhance="histogram")
image(cal_mix, cal5~x*y, key=T)

cal6 <- selectROI(cal_mix, mz = 287.4, contrast.enhance="histogram")
image(cal_mix, cal6~x*y, key=T)

cal_levels = makeFactor(L1 = cal1, L2 = cal2, L3 = cal3,
           L4 = cal4, L5 = cal5, L6 = cal6)
...
image(cal_response, subset=cal1)
...
# Read in as csv???
concs = c(10, 1, 0.1, 0.01, 0.001, 0.0001)
...
cal_list = list()
for(i in 1:nrow(mean_resp_cal)){
  mz = as.character(rownames(mean_resp_cal)[i])
  print(mz)
  
  ints = mean_resp_cal[i,]
  if(sum(!is.na(ints)) > 2){
    
    temp_df = data.frame(int = as.numeric(ints),
           sample = colnames(mean_resp_cal)) %>%
      mutate(conc = cal_metadata$conc[which(cal_metadata$sample == sample)])
    
    cal_list[[mz]] = lm(int~conc, data = temp_df)
  } else{
    cal_list[[mz]] = "NO DATA"
  }
}
